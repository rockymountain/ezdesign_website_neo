...
function validatePayload(payload: ContactPayload): Record<string, string> {
  ...
  if (!turnstileToken) errors.turnstileToken = 'Vui lòng hoàn tất xác minh chống spam.';

  if (name.length > MAX_LENGTH.name) errors.name = 'Tên quá dài.';
  if (contact.length > MAX_LENGTH.contact) errors.contact = 'Thông tin liên hệ quá dài.';
  if (interest.length > MAX_LENGTH.interest) errors.interest = 'Giải pháp quan tâm quá dài.';
  if (industry.length > MAX_LENGTH.industry) errors.industry = 'Ngành quá dài.';
  if (message.length > MAX_LENGTH.message) errors.message = 'Nội dung ghi chú quá dài.';
  if (sourcePath.length > MAX_LENGTH.sourcePath) errors.sourcePath = 'Source path quá dài.';
  if (userAgent.length > MAX_LENGTH.userAgent) errors.userAgent = 'User agent quá dài.';

  if (website) {
    const isValidWebsite = /^https?:\/\/.+/i.test(website);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (honeypot) {
      // Silently accept bot-like submissions, but do not store or notify.
      return jsonResponse(
        {
          ok: true,
    ...
    return jsonResponse(
      {
        ok: true,
        message: 'Cảm ơn bạn. EZDesign đã nhận được thông tin và sẽ phản hồi sớm.',
      },
      200,
      allowedOrigin,
    );
